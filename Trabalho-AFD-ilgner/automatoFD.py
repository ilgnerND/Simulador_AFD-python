import re, sys
	...
	##-----Método para ler o arquivo disponível-------
	##------------------------------------------------
def leArquivo():
	...
	##----Método para ler a linguagem da palavra onde consta:--------
	##----conjunto de símbolos (alfabeto) entre chaves--------------------
	##----conjunto de estados atingíveis, entre chaves--------------------
	##----caractere referente ao conjunto de regras de transição----------
	##----estado inicial--------------------------------------------------
	##----conjunto de estados finais, entre chaves------------------------
	##--------------------------------------------------------------------
def PrimeiraLinha(automato):
	...
	##----Método que busca o estado inicial do automato-------------------
	##--------------------------------------------------------------------
def buscaEstadoInicial(automato):
	...
	##----Mêtodo para ler as regras de transição definidas no arquivo-------------------
	##----Cada regra contém 3 (três) itens, estado inicial, símbolo e estado alvo,------
	##----e devem estar separados por vírgula-------------------------------------------
	##----As regras devem estar dispostas em linhas diferentes, uma logo após a outra---
	##
def leRegrasDeTransicao(automato):
	...
	##----Método que lê a palavra informada na execução-----------
	##----------------------------------------------------------
def leituraDaPalavra(estado_inicial, estados_finais, regras):
	...
	##-------Método que verifica se o automato que está--------------
	##-------sendo lido pelo programa, a palavra é valida ou não,-----
	##-------caso não seja ele solicita um ARQUIVO VALIDO pelo sistema.------------
...
def main():
	##-------Lê arquivo-----------------------------------
	automato = leArquivo()
	##-------Lê dados da primeira linha da lista----
	dados = PrimeiraLinha(automato)
	##-------Busca estado inicial-------------------------
	estado_inicial = buscaEstadoInicial(automato)
	##-------Lê as regras para transição------------------
	regras = leRegrasDeTransicao(automato)
	##-------Atribui os devidos elementos do automato-----------------
	alfabeto = dados[0]
	estados = dados[1]
	estados_finais = dados[2]
	##-------Imprime as informações da linguagem a ser lida------------------------
	...
	for regra in regras:
		print(regra)
	print()
	##-------Testa se a palavra e Aceita ou não Aceita-----------
...
if len(sys.argv) != 3:
	...
	print("Tente: python3 main.py <arquivo-txt> <palavra> || py main.py <arquivo-txt> <palavra>")
else:
	print(main())
